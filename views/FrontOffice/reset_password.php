...
if (isset($_GET['token'])) {
    $token = $_GET['token'];
    $stmt = $pdo->prepare("SELECT * FROM userss WHERE reset_token = ? AND token_expire > NOW()");
    $stmt->execute([$token]);
    ...
    if ($user) {
        if (isset($_POST['update'])) {
            ...
            $pdo->prepare("UPDATE userss SET password=?, reset_token=NULL, token_expire=NULL WHERE id=?")
